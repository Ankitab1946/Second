...
def read_all_sheets(uploaded_file):
    # returns dict: sheetname -> dataframe
    if uploaded_file is None:
        return {}
    try:
        x = pd.read_excel(uploaded_file, sheet_name=None, dtype=object)
        # normalize: ensure columns are strings
        for k,df in x.items():
...
def df_row_hash(df):
    # Create a stable row hash by concatenating stringified values
...
def compare_sheets(df_left, df_right, sheet_name, max_diffs=1000):
    ...
    Compare two dataframes and return:
      - summary dict
      - detail_df: columns [row_key, col, left_value, right_value, status]
    status values: 'same','changed','left_only','right_only'
    """
    summary = {
        "sheet": sheet_name,
        "left_rows": 0,
    ...
    # compute row hashes to detect added/removed rows irrespective of index
    ...
    # For detailed cell differences, align rows by hash where possible
    # Create mapping hash -> first index
    left_map = {h: i for i,h in enumerate(left_hash)}
    right_map = {h: i for i,h in enumerate(right_hash)}

    records = []
    cell_changes = 0
    # compare common rows (by hash intersection)
    ...
    # rows only in left (removed) and only in right (added) - we'll record the row content for visibility
    if cell_changes < max_diffs:
        # removed rows
...
def build_reports(dict_left, dict_right, max_diffs_per_sheet=1000):
    ...
    with pd.ExcelWriter(excel_buffer, engine='xlsxwriter') as writer:
        ...
        for s, df in detail_frames.items():
            # truncate sheet names to 31 chars for Excel
...
if file1 is not None and file2 is not None:
    ...
    if st.button("Run comparison"):
        ...
        # show sheet-level interactive selection
        ...
        st.markdown("---")
        st.markdown("Tips:\n- If your sheets have a natural key column (e.g., 'Account Code' or 'ID'), you can pre-sort both files by that column to get more meaningful row alignment.\n- For very large files consider increasing available memory or running the script on a server.")
...
# Footer
st.markdown("---")
st.caption("Made for GC Excel comparison — ask me to add features: choose key columns for alignment, fuzzy matching on headers, color-coded HTML, or auto email distribution of the report.")
