...
# """)
# =====================================================================
# AI Testcase Generator — Supports Claude 3.7 (Inference Profiles)
# =====================================================================
...
# PAGE CONFIG
# =====================================================================
st.set_page_config(layout="wide", page_title="AI Testcase Generator (Claude 3.7 + Profiles)")
st.title("AI Testcase Generator — Claude 3.7 Sonnet (Inference Profiles Enabled)")
...
def make_clients():
    ...
    return (
        session.client("bedrock", region_name=aws_region, config=cfg, verify=bedrock_verify),
        session.client("bedrock-runtime", region_name=aws_region, config=cfg, verify=bedrock_verify)
    )
...
# =====================================================================
# TESTCASE GENERATION LOGIC — CLAUDE 3.7
# =====================================================================
PROMPT_TEMPLATE = (
    "You are a senior QA engineer. Generate a JSON array of test cases. "
    "Each object MUST contain: id, title, priority, type, preconditions, steps, expected_result.\n\n"
...
def generate_testcases(runtime, profile_arn, summary, description, labels, comments, max_tokens):
    ...
    try:
        response = runtime.invoke_model(
            modelId=profile_arn,   # REQUIRED FOR CLAUDE 3.7
            contentType="application/json",
            accept="application/json",
    ...
    out = json.loads(raw)
    assistant_msg = out["output"]["message"]["content"][0]["text"]
    ...
    try:
        ...
        return [{"id": "ERR", "title": "Parse Error", "steps": [assistant_msg]}]
...
issue_key = st.text_input("Jira Issue Key (e.g., QA-101)")
...
if st.button("Fetch Jira Issue"):
    try:
        issue = fetch_jira_issue(
            jira_base, issue_key, jira_auth_method,
            email=jira_email, api_token=jira_api_token,
            username=jira_username, password=jira_password,
            verify=jira_verify
        )
...
# =====================================================================
# ISSUE PREVIEW + PROFILE SELECTION (Claude 3.7)
# =====================================================================
if "issue" in st.session_state:

    fields = st.session_state["issue"]["fields"]
    ...
    labels = fields.get("labels", [])
    comments = [c["body"] for c in fields.get("comment", {}).get("comments", [])]

    mgmt, runtime = make_clients()

    st.write("---")
    st.subheader("Step 3 — Select Claude 3.7 Inference Profile")

    ok, profiles, msg = list_inference_profiles(mgmt)
    if not ok:
        st.warning("Could not auto-load profiles. You may enter ARN manually.")

    profile_options = [f"{p['name']} — {p['arn']}" for p in profiles]
    profile_arns = [p["arn"] for p in profiles]

    choice = st.selectbox("Choose Inference Profile", ["-- Select --"] + profile_options)

    manual_profile = st.text_input("OR Enter Claude 3.7 Profile ARN manually")

    if choice != "-- Select --":
        selected_arn = profile_arns[profile_options.index(choice)]
    else:
        selected_arn = manual_profile.strip()

    if selected_arn:
        st.info(f"Using Profile ARN:\n`{selected_arn}`")

    # =================================================================
    # Generate Test Cases — Claude 3.7
    # =================================================================
    st.write("---")
    st.subheader("Step 4 — Generate Test Cases (Claude 3.7)")

    max_tokens = st.number_input("Max Tokens", value=1500)

    if st.button("Generate with Claude 3.7"):
        if not selected_arn:
            st.error("Select or enter profile ARN first.")
        else:
            try:
                tcs = generate_testcases(
                    runtime,
